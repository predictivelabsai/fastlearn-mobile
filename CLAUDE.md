...
36 test files, 370 tests. Three layers:
...
2. Test (370 tests with coverage)
...
Pull requests run analysis and tests without release secrets. Main-branch and manual release builds
require the four signing secrets, verify APK/AAB signatures, and fail instead of using a debug key.
The manual `.github/workflows/deploy-play-internal.yml` workflow always prepares a signed AAB and
can optionally upload only to Google Play's internal track. Uploading is disabled by default and
uses Workload Identity Federation; see `docs/google-play-release.md`.
